...
class BeforeCorruptionSampler(Sampler):
    ...
    def _get_episode_items(self):
        ...
        # source_items = torch.cat([self._sample_instances(source_items_per_label[label], self.n_source) for label in labels]) * self.n_domains + source_perturbation
        # target_items = torch.cat([self._sample_instances(target_items_per_label[label], self.n_target) for label in labels]) * self.n_domains + target_perturbation
        ...
        )  # the target items is the last half ones
