...
def make_episode_pipeline(
    dataset_spec_list: List[Union[HDS, BDS, DS]],
    split: Split,
    ...
    **kwargs,
    ...
    for i in range(len(dataset_spec_list)):
        episode_reader = reader.Reader(
            dataset_spec=dataset_spec_list[i], split=split, shuffle=data_config.shuffle
        )
            ...
            while sup_added < nb_support:
                ...
                if sample_dic["id"] not in used_ids:
                    ...
                    support_images.append(
                        self.transforms(sample_dic["image"]).unsqueeze(0)
                    )
                    sup_added += 1
