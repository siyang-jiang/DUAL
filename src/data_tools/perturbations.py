...
def to_int8np(func):
    def wrapper(x, severity_params, image_size):
        return np.clip(np.array(func(x, severity_params, image_size)), 0, 255).astype(
            np.uint8
        )
...
def gaussian_blur(x, severity_params, image_size):
    x = gaussian(np.array(x) / 255.0, sigma=severity_params, channel_axis=2)
    return np.clip(x, 0, 1) * 255
...
def glass_blur(x, severity_params, image_size):
    x = np.uint8(
        gaussian(np.array(x) / 255.0, sigma=severity_params[0], channel_axis=2) * 255
    )
    ...
    return (
        np.clip(gaussian(x / 255.0, sigma=severity_params[0], channel_axis=2), 0, 1)
        * 255
    )
...
@to_int8np
def jpeg_compression_old(x, severity_params, image_size):
    output = BytesIO()
...
def jpeg_compression(x, severity_params, image_size):
    buf = io.BytesIO()
    imageio.imwrite(buf, x, format="jpg", quality=severity_params)
    s = buf.getbuffer()
    return imageio.imread(s, format="jpg")


# TODO: replace some of these with TF transforms
@to_int8np
def pixelate(x, severity_params, image_size):
    x = TF.to_tensor(x)
    new_size = int(image_size / severity_params)
    x = TF.resize(x, new_size)
    x = TF.resize(x, image_size)
    x = TF.to_pil_image(x)
    return x
